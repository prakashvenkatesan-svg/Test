...
const isTemplateV3 = () =>
  ["account_opening_form_v3.pdf", "account_opening_form_v3l.pdf"].includes(
    getActiveTemplateFileName(),
  );
...
const getApplicantDeclarationSignatureFieldName = () =>
  getActiveTemplateFileName() === "account_opening_form_v3l.pdf"
    ? "Signature3"
    : "";
...
const getV3ApplicantWetSignatureOverlay = ({
  signatureImagePath = "",
  ...
  if (
    ...
    (!String(signatureImagePath || "").trim() &&
      !String(signatureImageBase64 || "").trim())
  ) {
    return null;
  }
  ...
  const overlay = {
    ...
    height: 28.07,
    padding: 2,
    imageBase64: String(signatureImageBase64 || "").trim(),
    imagePath: String(signatureImagePath || "").trim(),
  };

  console.log("[PDF] Applicant wet signature overlay enabled:", {
    pageIndex: overlay.pageIndex,
    x: overlay.x,
    y: overlay.y,
    width: overlay.width,
    height: overlay.height,
    hasImagePath: Boolean(overlay.imagePath),
    hasImageBase64: Boolean(overlay.imageBase64),
  });
...
const getTemplateCompatibilityOverlays = ({
  ...
  [
    ...
    (String(signatureImagePath || "").trim() ||
      ...
      : null,
    String(getApplicantDeclarationSignatureFieldName()) &&
    (String(signatureImagePath || "").trim() ||
      String(signatureImageBase64 || "").trim())
      ? {
          kind: "fieldImage",
          fieldName: getApplicantDeclarationSignatureFieldName(),
          imageBase64: String(signatureImageBase64 || "").trim(),
          imagePath: String(signatureImagePath || "").trim(),
          padding: 2,
        }
      : null,
    getV3ApplicantWetSignatureOverlay({
      signatureImagePath,
      signatureImageBase64,
    }),
