...
const buildKraAddressLines = (kra = {}, identity = {}) => {
  const hasCorAddress = !!(kra.app_cor_add1 || kra.app_cor_add2 || kra.app_cor_add3 || kra.app_cor_city || kra.app_cor_pincd);
  
  if (hasCorAddress) {
    const correspondenceState = resolveKraStateDisplayName(kra.app_cor_state, kra.app_per_state, identity.state);
    const correspondenceSecondaryLine = buildSecondaryAddressLine(kra.app_cor_add2, kra.app_cor_add3);
    
    let corAdd1 = firstNonEmpty(kra.app_cor_add1);
    let corAdd1Lines = [];
    if (corAdd1.length > 55) {
      let breakPoint = corAdd1.lastIndexOf(" ", 55);
      if (breakPoint === -1) breakPoint = corAdd1.lastIndexOf(",", 55);
      if (breakPoint === -1) breakPoint = 55;
      let line1 = corAdd1.substring(0, breakPoint).trim();
      let line2 = corAdd1.substring(breakPoint).trim();
      if (line2.startsWith(",")) line2 = line2.substring(1).trim();
      corAdd1Lines = [line1, line2].filter(Boolean);
    } else {
      corAdd1Lines = [corAdd1].filter(Boolean);
    }
    ...
    const correspondenceLines = [
      ...
      dedupeAddressParts([
        isSameAddressPart(correspondenceSecondaryLine, kra.app_cor_city) ? "" : kra.app_cor_city,
        correspondenceState,
        kra.app_cor_pincd,
      ]).join(", "),
  ...
  const hasPerAddress = !!(kra.app_per_add1 || kra.app_per_add2 || kra.app_per_add3 || kra.app_per_city || kra.app_per_pincd);
  
  if (hasPerAddress) {
    const permanentState = resolveKraStateDisplayName(kra.app_per_state, kra.app_cor_state, identity.state);
    const permanentSecondaryLine = buildSecondaryAddressLine(kra.app_per_add2, kra.app_per_add3);
    
    let perAdd1 = firstNonEmpty(kra.app_per_add1);
    let perAdd1Lines = [];
    if (perAdd1.length > 55) {
      let breakPoint = perAdd1.lastIndexOf(" ", 55);
      if (breakPoint === -1) breakPoint = perAdd1.lastIndexOf(",", 55);
      if (breakPoint === -1) breakPoint = 55;
      let line1 = perAdd1.substring(0, breakPoint).trim();
      let line2 = perAdd1.substring(breakPoint).trim();
      if (line2.startsWith(",")) line2 = line2.substring(1).trim();
      perAdd1Lines = [line1, line2].filter(Boolean);
    } else {
      perAdd1Lines = [perAdd1].filter(Boolean);
    }
    ...
    const permanentLines = [
      ...
      dedupeAddressParts([
        isSameAddressPart(permanentSecondaryLine, kra.app_per_city) ? "" : kra.app_per_city,
        permanentState,
        kra.app_per_pincd,
      ]).join(", "),
    ].filter((value) => String(value || "").trim() !== "");
...
const buildKraSummaryOverlays = (application) => {
  ...
  const addressStartY = 530; // Near top of Address box (473 + 70)
  
  return addressLines.slice(0, 4).map((line, index) => ({
    pageIndex: 41, // Page 42
    x: 192,
    y: addressStartY - index * 16,
    size: 9,
    text: line,
  }));
};
...
const getVerifiedIdentityDocumentFields = (application) => {
  ...
  if (resolveVerifiedIdentitySource(application) === "kra") {
    fields.ADDRESS = ""; // Clear native field so overlays can take over
  }
