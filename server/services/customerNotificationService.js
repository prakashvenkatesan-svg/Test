...
const buildAlternativeAbsolutePaths = (absolutePath) => {
  ...
  if (isLambda) {
    const forwardSlashPath = normalizedAbsolutePath.replace(/\\/g, "/");
    if (forwardSlashPath.includes("/uploads/")) {
      const idx = forwardSlashPath.indexOf("/uploads/");
      candidatePaths.add(path.join("/tmp", forwardSlashPath.slice(idx)));
    }
    if (forwardSlashPath.includes("/generated-pdfs/")) {
      const idx = forwardSlashPath.indexOf("/generated-pdfs/");
      candidatePaths.add(path.join("/tmp", forwardSlashPath.slice(idx)));
    }
  }
...
const resolveAbsolutePath = (inputPath = "") => {
  ...
  const isLambda = !!process.env.AWS_EXECUTION_ENV;
  const relativeJoined = path.join(PROJECT_ROOT, normalizedPath.replace(/^[/\\]+/, ""));
  const candidatePaths = [relativeJoined];
  
  if (isLambda) {
    const forwardSlashPath = normalizedPath.replace(/\\/g, "/");
    if (forwardSlashPath.includes("uploads/")) {
      const idx = forwardSlashPath.indexOf("uploads/");
      candidatePaths.push(path.join("/tmp", forwardSlashPath.slice(idx)));
    }
    if (forwardSlashPath.includes("generated-pdfs/")) {
      const idx = forwardSlashPath.indexOf("generated-pdfs/");
      candidatePaths.push(path.join("/tmp", forwardSlashPath.slice(idx)));
