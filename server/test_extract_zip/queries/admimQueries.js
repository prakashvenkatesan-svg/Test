const getAdminMetricsQuery = `
  SELECT
    ...
    COUNT(*) FILTER (WHERE assigned_admin_id IS NOT NULL)::int AS claimed_applications,
    COUNT(*) FILTER (WHERE assigned_admin_id IS NULL)::int AS unclaimed_applications
...
const getApplicationDetailQuery = `
  ...
  FROM (
    SELECT
      ...
      (
        SELECT COALESCE(json_agg(history_row ORDER BY history_row.created_at DESC), '[]'::json)
        FROM (
          SELECT
            ash.*,
          ...
          WHERE ash.application_id = ka.id
        ) AS history_row
      ) AS status_history,
      (
        SELECT row_to_json(assignment_row)
        FROM (
          SELECT
            aa.*,
          ...
          LEFT JOIN public.admin_users au
            ON au.id = aa.assigned_to
          WHERE aa.application_id = ka.id
          LIMIT 1
        ) AS assignment_row
      ) AS assignment
    FROM public.kyc_applications ka
...
const updateApplicationAssignedRmQuery = `
  UPDATE public.kyc_applications
  SET
    assigned_rm_id = $1,
    updated_at = NOW()
...
const upsertApplicationAssignmentQuery = `
...
const deleteApplicationAssignmentQuery = `
