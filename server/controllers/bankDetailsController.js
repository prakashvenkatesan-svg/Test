...
const getBankMasterMicrColumn = async () => {
  const result = await masterPool.query(
    ...
    WHERE table_schema = 'public'
      ...
      AND column_name IN ('micr_code', 'micr')
    ORDER BY CASE column_name
      WHEN 'micr_code' THEN 1
      WHEN 'micr' THEN 2
      ELSE 3
    END
    LIMIT 1
...
const fetchBankMasterDetails = async (ifscCode) => {
  const micrColumn = await getBankMasterMicrColumn();
  const micrSelect = micrColumn
    ? `, ${micrColumn} AS micr_code`
    : ", NULL::text AS micr_code";
