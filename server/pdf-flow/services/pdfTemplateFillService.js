...
const NOMINEE_EMAIL_FONT_SIZE = Number(
  process.env.ACCOUNT_OPENING_NOMINEE_EMAIL_FONT_SIZE || "5",
);
const MIN_NOMINEE_EMAIL_FONT_SIZE = Number(
  process.env.ACCOUNT_OPENING_NOMINEE_EMAIL_MIN_FONT_SIZE || "3.5",
);

const isNomineeEmailField = (fieldName = "") =>
  /^Email ID(\d+|_\d+)$/.test(String(fieldName || "").trim());
...
const getFieldFontSize = (
  ...
  if (isNomineeEmailField(normalizedFieldName)) {
    const baseFontSize = DEFAULT_FILLED_FONT_SIZE;

    if (!textFont || !String(value || "").trim()) {
      return NOMINEE_EMAIL_FONT_SIZE;
    }

    const [rect] = getFieldWidgetRects(pdfDoc, form, normalizedFieldName);
    const availableWidth = Number(rect?.width || 0) - 6;

    if (availableWidth <= 0) {
      return NOMINEE_EMAIL_FONT_SIZE;
    }

    const textWidthAtBaseSize = textFont.widthOfTextAtSize(
      String(value),
      baseFontSize,
    );

    if (!textWidthAtBaseSize || textWidthAtBaseSize <= availableWidth) {
      return baseFontSize;
    }

    const fittedFontSize = (availableWidth / textWidthAtBaseSize) * baseFontSize;

    return Math.max(
      MIN_NOMINEE_EMAIL_FONT_SIZE,
      Math.min(baseFontSize, fittedFontSize),
    );
  }

  return DEFAULT_FILLED_FONT_SIZE;
};
...
const embedImage = async (pdfDoc, { imageBase64, imagePath }) => {
  ...
  console.log("[PDF] Embed image source check:", {
    imagePath: String(imagePath || "").trim(),
    hasImageBase64: Boolean(String(imageBase64 || "").trim()),
    bytesLoaded: Number(imageBytes?.length || 0),
  });
...
const drawFieldImageOverlay = async (pdfDoc, form, overlay) => {
  ...
  for (const rect of rects) {
    ...
    // Check if the target box is landscape, but the image is portrait
    const isBoxLandscape = targetWidth > targetHeight;
    const isImagePortrait = embeddedImage.height > embeddedImage.width;
    const needsRotation = isBoxLandscape && isImagePortrait;

    const imageWidth = needsRotation ? embeddedImage.height : embeddedImage.width;
    const imageHeight = needsRotation ? embeddedImage.width : embeddedImage.height;
    ...
    const physicalDrawWidth = imageWidth * scale;
    const physicalDrawHeight = imageHeight * scale;

    const xPos = rect.x + padding + (targetWidth - physicalDrawWidth) / 2;
    const yPos = rect.y + padding + (targetHeight - physicalDrawHeight) / 2;

    drawJobs.push({
      pageIndex: rect.pageIndex,
      image: embeddedImage,
      x: xPos,
      // If rotated, anchor y is shifted up by the physical draw height
      y: needsRotation ? yPos + physicalDrawHeight : yPos,
      // `drawImage` expects the original unrotated dimensions
...
const setPdfFieldValue = (pdfDoc, form, fieldName, value, textFont) => {
  ...
  if (fieldType === "PDFTextField") {
    ...
    try {
      field.setFontSize(
        getFieldFontSize(pdfDoc, form, fieldName, value, textFont),
      );
    } catch (error) {
...
const normalizeAllTextFieldAppearances = (pdfDoc, form, textFont) => {
...
const fillPdfTemplate = async ({ templatePath, outputPath, payload }) => {
  ...
  for (const overlay of payload.overlays || []) {
    if (overlay?.kind === "fieldImage" || overlay?.kind === "image") {
      console.log("[PDF] Preparing image overlay:", {
        kind: overlay.kind,
        fieldName: overlay.fieldName || "",
        pageIndex:
          overlay.pageIndex === undefined || overlay.pageIndex === null
            ? null
        ...
        hasImagePath: Boolean(String(overlay.imagePath || "").trim()),
        hasImageBase64: Boolean(String(overlay.imageBase64 || "").trim()),
      });
      fieldImageDrawJobs.push(...(await drawFieldImageOverlay(pdfDoc, form, overlay)));
    }
  }

  normalizeAllTextFieldAppearances(pdfDoc, form, textFont);
  form.flatten();
  ...
  for (const job of fieldImageDrawJobs) {
    ...
    page.drawImage(job.image, {
      ...
      width: job.width,
      height: job.height,
      rotate: job.rotate ? degrees(job.rotate) : undefined,
    });
  }
