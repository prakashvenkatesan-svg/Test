...
const DEFAULT_OUTPUT_DIR = isLambda
  ? "/tmp/generated-pdfs"
  : path.join(
      __dirname,
      "..",
...
const prepareApplicationPdf = async (applicationId, options = {}) => {
  await ensureApplicationBoid(applicationId);
  
  // Ensure the Client Code is generated before creating the PDF
  try {
    await ensureClientCode(applicationId);
  } catch (err) {
    console.error("[PdfStepService] Error ensuring client code:", err);
  }
  ...
  const payload = buildPdfFieldPayload(application, options);
