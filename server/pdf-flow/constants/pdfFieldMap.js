...
const resolveLocalSignatureUploadPath = (applicationId) => {
  ...
  const candidateFolders = ["upload-signature", "signature-pad", "capture-photo"];

  for (const folderName of candidateFolders) {
    const folderPath = path.join(signatureRoot, folderName);
    ...
    const matchingFiles = fs
      ...
      .filter(
        (entry) =>
          entry.isFile() &&
          entry.name.startsWith(`${normalizedApplicationId}_`),
      )
      .map((entry) => {
        ...
        return {
          ...
          relativePath: `/uploads/signature/${folderName}/${entry.name}`,
        };
      })
      .sort((left, right) => right.mtimeMs - left.mtimeMs);

    if (matchingFiles[0]?.relativePath) {
      return matchingFiles[0].relativePath;
    }
  }
...
const buildNomineeColumnFields = (nominee = {}, index, fallbackAddressFields = {}, fallbackBoid = "") => {
  ...
  const nomineeFields = {
    [`NAME OF THE NOMINEES MRMS${suffix}`]: nominee.nominee_name || "",
    [`NAME OF THE NOMINEES MRMS_${index + 3}`]: nominee.nominee_name || "",
    [`1st nominee name`]: index === 1 ? nominee.nominee_name || "" : "",
    [`RELATIONSHIP WITH THE APPLICANT IF ANY${suffix}`]: nominee.relation || "",
    [`RELATIONSHIP WITH THE APPLICANT IF ANY_${index + 3}`]: nominee.relation || "",
    [`SHARE OF EQUALLY IF NOT EQUALLY PLEASE${suffix}`]: nominee.allocation_percentage ? `${nominee.allocation_percentage}%` : "",
    [`SHARE OF EQUALLY IF NOT EQUALLY PLEASE_${index + 3}`]: nominee.allocation_percentage ? `${nominee.allocation_percentage}%` : "",
...
const buildPdfFieldPayload = (application, options = {}) => {
  ...
  const resolvedSignatureImagePath = firstNonEmpty(
    ...
    resolveLocalSignatureUploadPath(application.id),
  );
  const resolvedSignatureImageBase64 = firstNonEmpty(
    signatureUpload.signature_base64,
  );
  console.log("[PDF] Resolved signature source:", {
    applicationId: application.id,
    signature_file_path: signatureUpload.signature_file_path || "",
    file_path: signatureUpload.file_path || "",
    fallback_signature_path: resolveLocalSignatureUploadPath(application.id),
    resolvedSignatureImagePath,
    hasSignatureBase64: Boolean(String(resolvedSignatureImageBase64 || "").trim()),
  });
  ...
  const fallbackClientId = boid.length >= 16 ? boid.replace(/\D/g, "").slice(-8) : "";
  ...
  const fields = {
    ...
    "DATE_3": formatDate(new Date()),
    "IPV Date": formatDate(new Date()),
    "Client UCC": clientCode,
    "DP INTERVAL REFERENCE NUMBER": clientCode,
    ...
    "MICR CODE":
      bank.micr_code || bank.micr || bank.bank_response?.micr_code || "",
    MICR: bank.micr_code || bank.micr || bank.bank_response?.micr_code || "",
  ...
  if (contact.email) {
    fields["EMAIL ID"] = toUpperText(contact.email);
  }
  ...
  const payload = {
    ...
    overlays: [
      ...getTemplateCompatibilityOverlays({
        ...
        signatureImageBase64: resolvedSignatureImageBase64,
      }),
  ...
  if (options.lat && options.lng) {
    payload.overlays.push({
      kind: "text",
      pageIndex: 0,
      x: 100,
      y: 55,
      text: `Lat Long: ${options.lat}, ${options.lng}`,
      size: 6,
    });
  }
