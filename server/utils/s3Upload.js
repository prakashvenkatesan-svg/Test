 ...
 * @returns {Promise<string|null>} - Presigned URL if successful, null otherwise
 */
const generatePresignedUrl = async (key, expiresIn = 3600) => {
  if (!bucketName) {
    console.warn("[S3] S3_BUCKET environment variable is not defined. Skipping presigned URL generation.");
    return null;
  }
  ...
  try {
    ...
    const command = new GetObjectCommand({
      Bucket: bucketName,
      Key: cleanKey,
      ResponseContentDisposition: `attachment; filename="${cleanKey.split('/').pop()}"`,
      ResponseContentType: "application/pdf",
    });

    return await getSignedUrl(client, command, { expiresIn });
  } catch (error) {
    console.error(`[S3] Failed to generate presigned URL for s3://${bucketName}/${cleanKey}:`, error.message);
    return null;
  }
};
