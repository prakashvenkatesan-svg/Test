...
const getFlexiEsignPositions = (application) => {
  // Check if DDPI is selected in the application (checking both ddpi_selected flag and personal_details value)
  const isDDPISelected = application.ddpi_selected === true ||
    (application.personal_details && application.personal_details.ddpi && application.personal_details.ddpi.toUpperCase() === 'YES');
  ...
  const basePositions = [
    ...
    createRect(11, 270, 295, 105, 30),  // Page 12: Details of disputes (moved left and up, smaller size)
    ...
    createRect(21, 90, 530, 130, 40),   // Page 22: 24/38 beside esign
    createRect(21, 90, 280, 130, 40),   // Page 22: 25/38 up esign
  ...
  if (process.env.EXTRA_ESIGN_PAGES) {
    ...
    extraPages.forEach(page => {
      ...
      if (page === 14) {
        // Page 14 special case: move bottom-right stamp further down to align with table
        basePositions.push(createRect(page - 1, 400, 10, 140, 45));
      } else {
